...
if [[ $# -ne 4 ]]; then
  echo "Usage: $0 <list> <Uniq-id> <'+' if positive,  '-' if negative> <'+' if plus strand, '-' if minus strand>";   
  exit;
fi
...
# make seq list
	# ls -1v $OUT_URL/tmp-$OUTPUT/seq > $OUT_URL/tmp-$OUTPUT/list-seq
	awk -F'\t' '{print $1 $2 ".fa"}' $S_LIST > $OUT_URL/tmp-$OUTPUT/list-seq

# make mfold features
	mfoldfile=$OUT_URL/tmp-$OUTPUT/list-seq
		while read line
		do
		cd $OUT_URL/tmp-$OUTPUT/run
		$MFOLD SEQ=$OUT_URL/tmp-$OUTPUT/seq/$line | grep Minimum |awk -F' ' '{print $5}' >> $OUT_URL/tmp-$OUTPUT/mid-result/$line
...
# make nucleotide 4, dinucleotide 16 features
		nucfile=$OUT_URL/tmp-$OUTPUT/list-seq
		while read line
		do
		python $OUT_URL/$M_NUC $OUT_URL/tmp-$OUTPUT/seq/$line >> $OUT_URL/tmp-$OUTPUT/mid-result/$line
		done < "$nucfile" 
...
# motif search - FIRE
python $FIRE_LEN $OUT_URL/tmp-$OUTPUT/dataset/data-all $OUT_URL/tmp-$OUTPUT/motif/list_new > $OUT_URL/tmp-$OUTPUT/fire-motif
...
then 
	...
	python $FIRE_LEN $OUT_URL/tmp-$OUTPUT/dataset/data-all $OUT_URL/tmp-$OUTPUT/motif/list_meme > $OUT_URL/tmp-$OUTPUT/meme-motif
	cp $GLAM_P10 $OUT_URL/tmp-$OUTPUT/motif/list_glam10 
	python $FIRE_LEN $OUT_URL/tmp-$OUTPUT/dataset/data-all $OUT_URL/tmp-$OUTPUT/motif/list_glam10 > $OUT_URL/tmp-$OUTPUT/glam10-motif
	cp $GLAM_P20 $OUT_URL/tmp-$OUTPUT/motif/list_glam20
	python $FIRE_LEN $OUT_URL/tmp-$OUTPUT/dataset/data-all $OUT_URL/tmp-$OUTPUT/motif/list_glam20 > $OUT_URL/tmp-$OUTPUT/glam20-motif
else
...
# motif search - TEISER
	teiserfile=$OUT_URL/tmp-$OUTPUT/list-seq
	while read line
	do
		$TEISER/Programs/scan_sequences_for_motif_profile -seedfile $TEISER_BIN -fastafile $OUT_URL/tmp-$OUTPUT/seq/$line -dataoutfile $OUT_URL/tmp-$OUTPUT/motif_t/$line-dat -reportfile $OUT_URL/tmp-$OUTPUT/motif_t/$line-rpt
	...
	numA=$(cat $OUT_URL/tmp-$OUTPUT/tmp-li |awk '{print $1}')
	numB=$( cat $OUT_URL/tmp-$OUTPUT/tmp-li | awk '{print $2}')
	nameL=$numA$numB
	if [ $STRAND == + ] 
	then 
		python $D_GENE_P $numA $numB | awk -F' ' '{print $7,$8,$9,$14,$15,$16}' >> $OUT_URL/tmp-$OUTPUT/mid-result/$nameL.fa 
	value=$(tail -n 1 $OUT_URL/tmp-$OUTPUT/seq/$nameL.fa) 
	python $STACK $value >> $OUT_URL/tmp-$OUTPUT/mid-result/$nameL.fa

	$KMER -R -d 4 $OUT_URL/tmp-$OUTPUT/seq/$nameL.fa $KMER_FA_P $KMER_OUT_P $OUT_URL/tmp-$OUTPUT/tmp-kmer
	else
	python $D_GENE_M $numA $numB | awk -F' ' '{print $7,$8,$9,$14,$15,$16}' >> $OUT_URL/tmp-$OUTPUT/mid-result/$nameL.fa 
	value=$(tail -n 1 $OUT_URL/tmp-$OUTPUT/seq/$nameL.fa) 
	python $STACK $value >> $OUT_URL/tmp-$OUTPUT/mid-result/$nameL.fa
	$KMER -R -d 4 $OUT_URL/tmp-$OUTPUT/seq/$nameL.fa $KMER_FA_M $KMER_OUT_M $OUT_URL/tmp-$OUTPUT/tmp-kmer	
	fi

	awk -F' ' '{print $2}' $OUT_URL/tmp-$OUTPUT/tmp-kmer >> $OUT_URL/tmp-$OUTPUT/mid-result/$nameL.fa
#	printf 0 >> $OUT_URL/tmp-$OUTPUT/mid-result/$nameL.fa
	rm $OUT_URL/tmp-$OUTPUT/tmp-kmer

	sed -i ':a;N;$!ba;s/\n/ /g' $OUT_URL/tmp-$OUTPUT/mid-result/$nameL.fa 
	rm $OUT_URL/tmp-$OUTPUT/tmp-li
	done < "$distfile"

# make train set	

trainfile=$OUT_URL/tmp-$OUTPUT/list-seq
while read line
do		
printf "@#$line @"
