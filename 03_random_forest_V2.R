...
#rfcv.res = rfcv(data_train[,c(2:144)], as.factor(data_train[,1]))
...
#import[1:138,,drop = FALSE]
#data_train.top <- data_train[,rownames(import)[1:30]]
...
import <- importance(fit.all, sort = TRUE)
import <- import[order(import, decreasing=TRUE),,drop = FALSE]
import[1:1104,,drop = FALSE]  #1152 (1104 for without distance)
data_train.top <- data_train[,rownames(import)[1:240]]
...
#fit
fit.top <- randomForest(as.factor(data_train$f0) ~., data=data_train.top, importance=TRUE, proximity=TRUE)

# Err rate plot
qplot(y=fit.top$err.rate[,1], xlab="Number of Trees", ylab="Error Rate")
...
data_train.top$f0 = data_train$f0
sub_train <- data_train.top[sample(nrow(data_train.top)), ]

k=5
n=floor(nrow(data_train.top)/k)
val_err=0
i=1
sub1 = ((i-1)* n+1)
sub2 = (i*n)
...
for(i  in 2:k)
{
  ...
  list_roc = roc(cross_val.test$response, Prediction_crossval[,2])
  val_err[i] = list_roc$auc
  plot.roc(list_roc, legacy.axes=TRUE, print.auc = TRUE, add = TRUE, col = i, print.auc.y=0.5-(i-1)*0.075, print.auc.x=0.30)
}
...
rocobj <- plot.roc(cross_val.test$response, Prediction_crossval[,2], 
                   ...
                   ci=TRUE, of="se",specificities=seq(0, 100, 5), ci.type="shape", ci.col="#2980B9") 

plot(ci.sp(rocobj, sensitivities=seq(0, 100, 5)), type="bars") 

#predict test data
data_test <- data_test[sample(nrow(data_test)), ]  
Prediction.test <- predict(fit.top, data_test, type="prob")
...
box_plot <- data.frame(Class = data_test$label, Prediction = Prediction.test[,2])
ggplot(data = box_plot, aes(Class, Prediction, color=Class)) + geom_boxplot(lwd=0.8, notch = TRUE)

## plot reference: pROC, PASIFIC
